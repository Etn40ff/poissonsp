...
\begin{itemize}
    ...
    that correspond to frozen ones in BFZ to get a new seed $\Sigma'$.
    \item Show that $h^*$ gives a quasi isomorphism (Fraser) from $\Sigma'$ 
    ...
    \item observe that $h^*$ and mutations commute so get a foldable seed for the non-standard structure.
      ...
      This structure is not yet the one we want: we need to unfreze the variables we temporarely froze.
      The hard part of this is only discussing the arrows among the variales we unfreze.
      One option here is to give explicit exchange relations that these variables satisfy in the cluster we have. 
      
      Idea for a trick: observe that when evaluating on $g$ these exchange relations can be made to use at most one function for each orbit.
      Is this enough?
    \item Show that $h^*$ and folding commute, so we get the seed we wanted.
\end{itemize}
    \item Show it is regular.

    If we new that folding "works" on the level of cluster algebras, this would be for free. 
    
    Alternatively, we use the starfish lemma: folding of regular functions is regular.
